...
abstract class AbstractPlugin extends SlimPlugin
{
    ...
    public function links_filter($links)
    {
        ...
        $plugin_links = ['<a target="_blank" href="' . $support_link . '">' . esc_html__('Support', 'checkout-fields-for-blocks') . '</a>'];
        $links = array_merge($plugin_links, $links);
        if ($this->docs_url) {
            $plugin_links = ['<a target="_blank" href="' . $this->docs_url . '">' . esc_html__('Docs', 'checkout-fields-for-blocks') . '</a>'];
            $links = array_merge($plugin_links, $links);
        }
        if ($this->settings_url) {
            $plugin_links = ['<a href="' . $this->settings_url . '">' . esc_html__('Settings', 'checkout-fields-for-blocks') . '</a>'];
            $links = array_merge($plugin_links, $links);
        }
        return $links;
    }
