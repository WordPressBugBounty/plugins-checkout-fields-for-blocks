...
if (!\class_exists('CBFieldsVendor\WPDesk_Tracker')) {
    /**
     * Class WPDESK_Tracker
     */
    class WPDesk_Tracker implements \WPDesk_Tracker_Interface
    {
        ...
        public function admin_enqueue_scripts(): void
        {
            ...
            if ($screen->id === 'admin_page_wpdesk_tracker' || $screen->id === 'admin_page_wpdesk_tracker_deactivate') {
                ...
                \wp_localize_script(self::ADMIN_HANDLER, self::ADMIN_LOCALIZE, ['action' => \esc_attr(self::AJAX_ACTION), 'nonce' => \wp_create_nonce(self::AJAX_NONCE), 'ajax_url' => \esc_url_raw(\admin_url('admin-ajax.php')), 'tracker_page' => \esc_url_raw(\str_replace("&amp;", "&", \admin_url(\wp_nonce_url("plugins.php?action=deactivate&plugin=" . $plugin . "&plugin_status=all&", "deactivate-plugin_" . $plugin)))), 'plugin' => \esc_attr($plugin), 'plugin_name' => \esc_attr($plugin_name), 'submit_txt' => \esc_html__("Submit &amp; Deactivate", "checkout-fields-for-blocks"), 'deactivation_plugins' => \wp_json_encode($deactivation_plugins)]);
            }
        }
